    ...
    foreach ctry in $datasets {
		...
		*** merge
		 ...
		 *** recode dim
			 ...
			 replace region = substr(region2, 5, 200) if inlist(region_c, 13, 14, 15, 16, 17, 18, 19, 20, 21, 64, 65)
			 *Lienchiang county (3 observations), outland and "head inactive" go to missing
			 replace region="" if inlist(region_c, 59, 80, 99)
			 decode sex, gen(sex2)
			 drop sex
		 ...
		 *** keep vars
			 keep hid pid dname cname hwgt pwgt ppopwgt region sex wealth educ educlev edyrs educ_c enroll age illiterate
...
*** Edu info: updated with https://wenr.wes.org/2016/06/education-in-taiwan 6-3-3 starting at 6
		* Primary
		local primaryage0 = 6
	...
	*** Keep vars
		keep cname hwgt pwgt ppopwgt region sex wealth ///
...
program define doedumean
   ...
   ***  Global
	...
	*** 1d
   di "---- 1d ----"	
        foreach x in  sex wealth region  {
		   di "----`x'----"	
...
*====
***** Run progs
local surveycodes "tw13 tw16"

local survey_year "2013 2016"
...
global varsp "hid pid dname cname pwgt ppopwgt age sex educ illiterate edyrs educ_c educlev enroll"
domerge
doeduc	
