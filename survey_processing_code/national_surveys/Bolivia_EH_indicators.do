...
cd "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Bolivia\Base EH2019"
usespss EH2019_Persona.sav
save persona.dta

*Calulating wealth variables

use "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Bolivia\Base EH2019\persona.dta"
*using monthly per capita household income 
xtile hhwealthindex = yhogpc [aw=factor], nquantiles(5)
save Bolivia_2019.dta, replace
...
label define depto 1 "Chuquisaca" 2 "La Paz" 3 "Cochabamba" 4 "Oruro" 5 "Potosí" 6 "Tarija" 7 "Santa Cruz" 8 "Beni" 9 "Pando", replace
...
************************************************************************************************************
cd "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Bolivia\Base EH2019"
use Bolivia_2019.dta, clear 

gen country_year="Bolivia"+"_"+"2019"
gen year=2017
gen iso_code2="BO"
gen iso_code3="BOL"
gen country = "Bolivia"
...
//
// 11.NINGUNO																			11	0
// 12.CURSO DE ALFABETIZACIÓN															12	1
// 13.EDUCACIÓN INICIAL O PRE-ESCOLAR (PRE KINDER/KINDER)								13	0
// 21.BÁSICO (1 A 5 AÑOS) - SISTEMA ESCOLAR ANTIGUO										21	1
// 22.INTERMEDIO (1 A 3 AÑOS) - SISTEMA ESCOLAR ANTIGUO									22	2
// 23.MEDIO (1 A 4 AÑOS) - SISTEMA ESCOLAR ANTIGUO										23	3
// 31.PRIMARIA (1 A 8 AÑOS) - SISTEMA ESCOLAR ANTERIOR									31	1, depends s05a_02c
// 32.SECUNDARIA (1 A 4 AÑOS) - SISTEMA ESCOLAR ANTERIOR								32	3
// 41.PRIMARIA (1 A 6 AÑOS) - SISTEMA ESCOLAR ACTUAL									41	1
// 42.SECUNDARIA (1 A 6 AÑOS) - SISTEMA ESCOLAR ACTUAL									42	2, depends s05a_02c
// 51.EDUCACIÓN BÁSICA  DE   ADULTOS (EBA) - EDUCACIÓN DE ADULTOS (SISTEMA ANTIGUO)		51	1
// 52.CENTRO DE EDUCACIÓN MEDIA  DE ADULTOS (CEMA) - EDUCACIÓN DE ADULTOS (SISTEMA 		52	2
// 61.EDUCACIÓN JUVENIL ALTERNATIVA (EJA) - EDUCACIÓN ALTERNATIVA Y ESPECIAL			61	1
// 62.EDUCACIÓN PRIMARIA DE ADULTOS (EPA) - EDUCACIÓN ALTERNATIVA Y ESPECIAL			62	1
// 63.EDUCACIÓN SECUNDARIA DE ADULTOS (ESA) - EDUCACIÓN ALTERNATIVA Y ESPECIAL			63	2
// 64.PROGRAMA NACIONAL DE POST ALFABETIZACIÓN - EDUCACIÓN ALTERNATIVA Y ESPECIAL		64	1
// 65.EDUCACIÓN  ESPECIAL - EDUCACIÓN ALTERNATIVA Y ESPECIAL							65	1
// 71.NORMAL (ESCUELA SUP. DE FORMACIÒN DE  MAESTROS) - EDUCACIÓN SUPERIOR				71	4
// 72.UNIVERSIDAD - EDUCACIÓN SUPERIOR													72	6
// 73.POSTGRADO DIPLOMADO - EDUCACIÓN SUPERIOR											73	5
// 74.POSTGRADO MAESTRÍA - EDUCACIÓN SUPERIOR											74	7
// 75.POSTGRADO DOCTORADO - EDUCACIÓN SUPERIOR											75	8
// 76.TÉCNICO DE UNIVERSIDAD - EDUCACIÓN SUPERIOR										76	4
// 77.TÉCNICO DE INSTITUTO TÉCNICO E INSTITUTO TECNOLOGÓGICO  (Duración mayor o igu		77	4
// 78.FORMACION SUPERIOR ARTÍSTICA - EDUCACIÓN SUPERIOR									78	4
// 79. INSTITUTOS DE FORMACIÓN MILITAR Y POLICIAL - EDUCACIÓN SUPERIOR					79	4
// 80. EDUCACIÓN TÉCNICA DE ADULTOS (ETA) - EDUCACIÓN SUPERIOR							80	4
// 81.OTROS CURSOS  (Duración menor a 2 años) - EDUCACIÓN SUPERIOR						81	4

local highestlevelattended s05a_02a 

recode `highestlevelattended' (11 13=0) (12 21 41 51 61 62 64 65=1) (22 52 63=2) (23 32=3) (71 76 77 78 79 80 81=4) (73=5) (72=6) (74=7) (75=8) , gen(highestlevelattended)
// replace highestlevelattended=1 if s05a_02a==31 & inlist(s05a_02c, 1, 2, 3, 4, 5, 6)
// replace highestlevelattended=2 if s05a_02a==31 & inlist(s05a_02c, 7, 8)
// replace highestlevelattended=1 if s05a_02a==42 & inlist(s05a_02c, 1, 2)
// replace highestlevelattended=2 if s05a_02a==42 & inlist(s05a_02c, 3, 4, 5, 6)
*6-2-4 replace with años de estudio
replace highestlevelattended=0 if inlist(aestudio, 0, 1, 2, 3, 4, 5)
replace highestlevelattended=1 if inlist(aestudio, 6, 7)
replace highestlevelattended=2 if inlist(aestudio, 8, 9, 10, 11)
replace highestlevelattended=3 if inlist(aestudio, 12, 13)
...
gen eduyears=aestudio


 
*DATA COLLECTION DAYS OF THE SURVEY
*http://anda.ine.gob.bo/index.php/catalog/84#metadata-data_collection
*Data collection took place between october and december of 2019. 


*According to some web: 
*Dates:  The school year begins in February. 
*https://www.reduno.com.bo/nota/confirman-las-fechas-de-inicio-del-calendario-escolar-2020-en-todo-el-pais-202017760
...
*100% of households have a difference of over 6 months=> no adjustment 
...
*(ii) 1 year before the official entrance age to primary. THIS
	 ...
	 generate attend_preschool   = 1 if s05a_06a == 1 // enrolled in preschool 
	 replace attend_preschool    = 0 if s05a_04 == 2 //not enrolled in current year in anything
	 replace attend_preschool    = 0 if s05b_10 == 2 //not assisting
...
generate attend = 1 if cmasi == 2
replace attend  = 0 if cmasi == 3 //not assisting despite being enrolled (few)
replace attend  = 0 if cmasi == 1 // not enrolled
...
***
generate high_ed = 1 if inlist(s05a_06a, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81) 
...
capture replace eduout  = . if (attend == 1 & s05a_06a == .) | age == . 
...
generate edu0 = 0 if s05a_02a>11
replace edu0  = 1 if s05a_02a==11
replace edu0  = 1 if inlist(s05a_02a, 13) //preschool
...
*Completion of higher
	foreach X in 2 4 {
		...
		replace comp_higher_`X'yrs = . if inlist(s05a_02a, .)
	}
...
gen overage2plus= 0 if attend==1 & inlist(s05a_06a, 41)
*There are 6 years of primary
	local i=0
    foreach grade of numlist 1/6 {
				local i=`i'+1
				replace overage2plus=1 if s05a_06c==`grade' & schage>prim_age0+1+`i'
                 }

* Literacy, tested on 14+ years old people
*SELF REPORTED: can you read and write?
recode s05a_01 (2=0) (1=1), gen(literacy)
replace literacy=. if age>14
...
compress
cd "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Bolivia\Base EH2019"
save Bolivia_microdata.dta, replace
...
gen year="2019"
gen country_year="Bolivia"+"_"+year
...
replace category="total" if category==""
	...
	*For Bolivia there are some individuals with unknown wealth quintiles. Drop the observations of this 'false category'
...
save "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Bolivia\Base EH2019\indicators_Bolivia_2019.dta", replace
