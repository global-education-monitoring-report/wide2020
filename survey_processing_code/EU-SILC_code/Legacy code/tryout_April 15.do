...
for X in any prim lowsec upsec: gen c_X=0 if t_pe040!=.
replace c_prim=1 if t_pe040>=1 & c_prim==0
replace c_lowsec=1 if t_pe040>=2 & c_lowsec==0
replace c_upsec=1 if t_pe040>=3 & c_upsec==0

*Age limits
foreach X in prim lowsec upsec {
foreach AGE in age {
	gen c_`X'_v2=c_`X' if `AGE'>=`X'_age1+3 & `AGE'<=`X'_age1+5
}
}
...
collapse comp_lowsec_2024 comp_upsec_2024 comp_prim_v2 comp_lowsec_v2 comp_upsec_v2 c_prim_v2 c_lowsec_v2 c_upsec_v2 *age0 *age1 *dur presch* [iw=hhweight], by(country year)
