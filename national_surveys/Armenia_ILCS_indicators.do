...
use hh2018.dta
*hh has region var called marz and settlement for urbanrural also totincome
*wealth 
xtile hhwealthindex = totincome [aw=weight], nquantiles(5)
merge 1:m recno using indiv2018.dta

egen indivrec=concat(recno memnum), punct("-")
save Armenia_2018.dta, replace

**WARNING! THE DATASETS FROM Armenia's NSO has some variables missing ON BOTH, I'm getting the grade variable merging it to the Armenian NSO dataset using the WB downloaded dataset -_- 
**WARNING! THE INDIVIDUAL DATASETS FROM NSO WEB AND WB WEB DO NOT HAVE THE SAME N° OF OBS
*IM MERGING FROM A DATASET OF 18 496 TO ONE OF 17 171


clear
use "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Armenia\ARM_2018_ILCS_v01_M_STATA\mem.dta"
*this is the dataset from WB
keep recno memnum e1_5 e1_6 e1_7 e2_2 e2_3 e2_4 e2_5 e2_6 e2_11 e2_12 e2_15 e2_16 e2_17 e2_18 e2_21 e2_22 e2_33 e2_35 e3_2 e3_3 e3_4 e3_5 e3_6 e3_7 e3_9
egen indivrec=concat(recno memnum), punct("-")
duplicates report indivrec 
merge 1:1 indivrec using "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Armenia\Armenia_2018.dta", gen(wbnsomerge)
drop if wbnsomerge==1

save "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Armenia\Armenia_2018.dta", replace
...
use "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Armenia\Armenia_2018.dta", clear

local country Armenia 
local sex a1_1
local urbanrural settlement // location
local hhweight weight
local age age
...
*clonevar age=`age' // its already called age
...
// 0<-1	no primary, illiterate
// 0<-2	no primary, literate
// 1<-3	primary
// 2<-4	general (basic)
// 3<-5	secondary
*changing here 
// 2<-6	preliminary vocational (handicraft, ind
// 4<-7	middle vocational (technical college, c
// 9<-8	higher (bachelor degree, master degree)
// 7<-9	post-graduate


local highestlevelattended a1_10

recode `highestlevelattended' (1=0) (2=0) (3=1) (4=2) (5=3) (6=2) (7=4) (8=9) (9=7) , gen(highestlevelattended)
...
replace highestlevelattended=1  if inlist(a1_10, 3, 4) & inlist(e2_3, 4, 5, 6, 7, 8) // primary and lowsec incomplete
replace highestlevelattended=2  if inlist(a1_10, 4, 5) & inlist(e2_3, 9 , 10, 11) // lowsec or basic general
replace highestlevelattended=3  if inlist(a1_10, 4, 5) & inlist(e2_3, 12) // secondary
...
// 2<-4	general (basic)
// 3<-5	secondary
// 3<-6	preliminary vocational (handicraft, ind
// 4<-7	middle vocational (technical college, c
// 9<-8	higher (bachelor degree, master degree)
// 7<-9	post-graduate
...
gen eduyears=.
replace eduyears=e2_3 if inlist(a1_10, 1, 2)  // no primary
replace eduyears=e2_3  if inlist(a1_10, 3) // primary
replace eduyears=e2_3  if inlist(a1_10, 4) // lowsec or basic general
replace eduyears=e2_3  if inlist(a1_10, 5) // secondary
replace eduyears=9+e2_3 if inlist(a1_10, 6) // preliminary vocational, lasts 3y line 11 of ISCED file
replace eduyears=9+3+e2_3  if inlist(a1_10, 7) // middle vocational, lasts 2y line 14 of ISCED 
replace eduyears=12+e2_3  if inlist(a1_10, 8) // higher (bachelor degree, master degree), assume bachelor
replace eduyears=12+4+e2_3  if inlist(a1_10, 9) // post-graduate, assume master
 
*DATA COLLECTION DAYS OF THE SURVEY
*This survey took place during all 2019, no go in a month by month basis 

*According to some web school starts in september
*https://eurasianet.org/armenian-kids-head-back-to-school-despite-smoldering-covid-19-outbreak
...
generate attend = 1 if e2_7 == 1
replace attend  = 0 if e2_7 == 2
...
// 7<-9	post-graduate
***
generate high_ed = 1 if inlist(a1_10, 7, 8, 9) & e2_7==1
...
capture replace eduout  = . if (attend == 1 & e2_11 == .) | age == . 
*capture replace eduout  = 1 if inlist(a1_10, 1, 2)  
...
***
generate edu0 = 0 if inlist(a1_10, 3, 4, 5, 6, 7, 8, 9)
replace edu0  = 1 if inlist(a1_10, 1, 2)
replace edu0  = 1 if inlist(e2_2, 0)
...
*Completion of higher
	foreach X in 2 4 {
		...
		replace comp_higher_`X'yrs = . if inlist(a1_10, .)
	}
...
compress
cd "C:\Users\taiku\Documents\GEM UNESCO MBR\Datasets to update WIDE\Other surveys\Armenia"
save Armenia_microdata.dta, replace
...
gen year="2018"
gen country_year="Armenia"+"_"+year
