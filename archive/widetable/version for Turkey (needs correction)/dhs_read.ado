...
program define dhs_read
	...
	*MR IS NOT AVAILABLE IN TURKEY 2013
	*local modules ir mr 
	local modules ir  
	cd "`output_path'"
	...
	if ("`country_name'" == "Turkey" |"`country_name'" == "Nicaragua" | "`country_name'" == "VietNam" | "`country_name'" == "Yemen"){
	local modules ir
	}
	foreach module of local modules {
		...
		foreach file of local filepath {
			cd "`data_path'/DHS/"
			*read a file
			use *v001 *v002 *v130 *v131 *v150 *s116  using "`file'", clear
			set more off
			
			rename *, lower
			for X in any v001 v002 v130 v131 v150 *s116 : capture rename mX X
			for X in any v001 v002 v130 v131 v150 *s116  : capture generate X=.
...
// 		*append using `r(files)', force
	...
	*but this is empty
	rename s116 ethnicity
	*ethnicity is now mother tongue 
	...
	* dhs variables to keep last
	...
	cd "`data_path'/DHS/"
	local filepath "Turkey/2013/TRPR62FL.dta"
	display "a ver si se corrige"
	display "`filepath'"
	* read all files 
	foreach file of local filepath {
