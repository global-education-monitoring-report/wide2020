...
### Grade 2
pasec2$COUNTRY <- cnt[match(pasec2$PAYS, cnt$pasec), "country"]
...
### Grade 6
pasec6$COUNTRY <- cnt[match(pasec6$PAYS, cnt$pasec), "country"]
...
pasec6_m <- ilsa_sum(pvnames = "MATHS", 
                     cutoff =   c(433.3, 521.5, 609.6), #68.1,
                     config = pasec_conf,
                     data = pasec6,
                     year = 2014,
                     level = "Primary",
...
pirls$COUNTRY <- cnt[match(pirls$IDCNTRY, cnt$iso_num), "country"]
...
## Create country id
names(wide_data)[1] <- "country"


ifelse(is.na(wide_data$iso_code3) )
