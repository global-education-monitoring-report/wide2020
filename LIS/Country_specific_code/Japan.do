...
*** Edu info: updated
	...
	*updating this according to database
		...
		*theres a big chunk of ppl with 'still in education' on educ_c, completing with educlev
		g comp_prim_temp=.
		replace comp_prim_temp=1 if inlist(educlev, 120, 210, 311, 312, 313)
		
		recode educ_c (12=0) (11 21/62=1), gen(comp_lowsec_temp)
		replace comp_lowsec_temp=1 if inlist(educlev, 210, 311, 312, 313)
		replace comp_lowsec_temp=0 if inlist(educlev, 120)

		recode educ_c (11 12 22=0) (21 31/62=1), gen(comp_upsec_temp) 
		replace comp_upsec_temp=1 if inlist(educlev, 210, 311, 312, 313)
		replace comp_upsec_temp=0 if inlist(educlev, 120)
		
		recode educ_c (11/32 42 52 62=0) (41 51 52 61=1), gen(comp_higher_temp) 
		replace comp_higher_temp=1 if inlist(educlev, 311, 312, 313)
		replace comp_higher_temp=0 if inlist(educlev,210, 120)
...
*====
***** Run progs
local surveycodes "jp04 jp09 jp11 jp14"

local survey_year "2004 2009 2011 2014"
