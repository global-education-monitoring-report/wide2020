...
gen year_c=hv007
replace year_c=year_c-1 if year_c>=2018 // I only have data on school calendar until 2017
...
replace round_dhs="4" if country_year=="Vietnam_2002" // fixing if not fixed before
...
save "$data_dhs\PR\dhs_collapse_by_categories_v8.dta", replace

export delimited "$data_dhs\PR\dhs_collapse_by_categories_v8.csv", replace
