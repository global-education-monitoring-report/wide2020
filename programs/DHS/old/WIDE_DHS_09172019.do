...
global data_raw_dhs "$gral_dir\WIDE\Data\DHS"
global programs_dhs "$gral_dir\WIDE_DHS_MICS\programs\dhs"
global programs_dhs_aux "$programs_dhs\auxiliary"
global aux_data "$gral_dir\WIDE_DHS_MICS\data\auxiliary_data"
global data_dhs "$gral_dir\WIDE_DHS_MICS\data\dhs"
...
global programs_dhs "$gral_dir\WIDE\WIDE_DHS_MICS\programs\dhs"

global programs_dhs_aux "$gral_dir\WIDE\WIDE_DHS_MICS\programs\dhs\auxiliary"
global aux_data "$gral_dir\WIDE\data_created\auxiliary_data"
global data_dhs "$gral_dir\WIDE\WIDE_DHS_MICS\data\dhs"
...
global vars_keep_dhs "hhid hvidx hv000 hv001 hv002 hv003 hv005 hv006 hv007 hv008 hv016 hv009 hv024 hv025 hv270 hv102 hv104 hv105 hv106 hv107 hv108 hv109 hv121 hv122 hv123 hv124 hv129"
...
*----------------------------------------------------------------------------------------------------
set more off
cap mkdir "$data_dhs\PR\countries\\part`num'"
...
*codebook individual_id // uniquely identifies
...
set more off
*cap use "C:\Users\Rosa_V\Desktop\casa\Step0_part1.dta", clear
*foreach part in part1 part2 {
foreach part in part1 part2 {
use "$data_dhs\PR\Step0_`part'.dta", clear
...
*save "C:\Users\Rosa_V\Desktop\casa\Step1_part1.dta", replace
}
...
keep iso hv005 hv104 country* year hv105 hv12* year* 
ren hv105 age
ren hv005 hhweight
replace hv122=. if hv122==8|hv122==9

for X in any 2002 2007 2012: tab hv121 if country_year=="Jordan_X"
for X in any 2002 2007 2012: tab hv122 if country_year=="Jordan_X"
...
for X in any preschool primary secondary higher: gen attend_X=0 if attend==1
replace attend_preschool =1 if hv122==0 & attend_preschool ==0 
replace attend_primary   =1 if hv122==1 & attend_primary   ==0 
replace attend_secondary =1 if hv122==2 & attend_secondary ==0
replace attend_higher    =1 if hv122==3 & attend_higher    ==0
for X in any preschool primary secondary higher: replace attend_X=. if (attend==.|hv122==.)

*sum no_attend attend at_* if age==10, separator(11)

collapse no_attend attend*  [iw=hhweight], by (iso_code3 country_year country year age)
...
*Dropping adjusted and unadjusted ages
cap drop *ageU *ageA
...
merge m:m hh_id using "$data_dhs\ethnicity_religion.dta", keepusing (ethnicity religion)
...
*Dropping variables
drop hhid hvidx individual_id cluster lowsec_age0 upsec_age0 prim_age1 lowsec_age1 upsec_age1 eduyears adjustment
...
save "$data_dhs\PR\dhs_collapse_by_categories_v7.dta", replace

export delimited "$data_dhs\PR\dhs_collapse_by_categories_v7.csv", replace


use "$data_dhs\PR\dhs_collapse_by_categories_v7.dta", clear
drop *no
